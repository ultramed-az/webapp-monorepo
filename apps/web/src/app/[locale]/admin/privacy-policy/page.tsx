...
export default function AdminPrivacyPolicyPage() {
  ...
  const loadPage = async () => {
    ...
    try {
      const page = await getAdminContentPage('privacy-policy');
      if (!page) {
        throw new Error('privacy-policy content tapılmadı.');
      }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await updateAdminContentPage('privacy-policy', {
      ...
      setSuccessMessage('Məxfilik siyasəti yeniləndi.');
      await loadPage();
    } catch (error) {
